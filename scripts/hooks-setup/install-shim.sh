...
CLI_PATH="$ROOT_DIR/scripts/hooks.ps1"
INSTALL_DIR="${HOOKS_BIN_DIR:-$HOME/.local/bin}"
VERIFY_ONLY=0
REMOVE=0
MARKER="ai-toolkit hooks shim"

usage() {
  cat <<'USAGE'
Usage:
  ./scripts/hooks-setup/install-shim.sh [--install-dir <dir>]
  ./scripts/hooks-setup/install-shim.sh --verify-only [--install-dir <dir>]
  ./scripts/hooks-setup/install-shim.sh --remove [--install-dir <dir>]
USAGE
}
