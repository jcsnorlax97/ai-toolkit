...
CLI_PATH="$ROOT_DIR/scripts/apply-preset.sh"
INSTALL_DIR="${PRESET_BIN_DIR:-$HOME/.local/bin}"
VERIFY_ONLY=0
REMOVE=0
MARKER="ai-toolkit apply-preset shim"

usage() {
  cat <<'USAGE'
Usage:
  ./scripts/presets/install-shim.sh [--install-dir <dir>]
  ./scripts/presets/install-shim.sh --verify-only [--install-dir <dir>]
  ./scripts/presets/install-shim.sh --remove [--install-dir <dir>]
USAGE
}
...
SHIM_PATH="$INSTALL_DIR/apply-preset"
